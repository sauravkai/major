...
export const LoginPage = () => {
  const { login, loginAsDemoRole, loginWithGoogle } = useAuth();
    ...
    <div className={`min-h-screen flex items-center justify-center py-12 relative overflow-hidden ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      >
        <motion.div variants={itemVariants} className="text-center space-y-2">
          ...
          <h2 className={`text-3xl font-heading font-extrabold ${isDark ? 'text-white bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-300' : 'text-slate-900 bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600'}`}>Welcome Back</h2>
        ...
        <motion.div 
          ...
          className={`relative p-6 rounded-3xl space-y-4 backdrop-blur-xl overflow-hidden ${isDark ? 'bg-gradient-to-r from-indigo-900/40 via-purple-900/40 to-pink-900/40 border-2 border-amber-500/30' : 'bg-gradient-to-r from-amber-50 via-purple-50 to-amber-50 border-2 border-amber-300/50'}`}
        >
          {/* Animated glow effect */}
          <div className={`absolute inset-0 animate-pulse ${isDark ? 'bg-gradient-to-r from-amber-500/10 via-purple-500/10 to-amber-500/10' : 'bg-gradient-to-r from-amber-200/20 via-purple-200/20 to-amber-200/20'}`}></div>
          <div className="relative z-10">
            ...
            >
              <Sparkles className={`w-5 h-5 animate-pulse ${isDark ? 'text-amber-400' : 'text-amber-600'}`} />
              <span className="text-sm font-mono uppercase tracking-widest font-bold bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-400 bg-clip-text text-transparent">
                Quick Demo Access
              </span>
              <Sparkles className={`w-5 h-5 animate-pulse ${isDark ? 'text-amber-400' : 'text-amber-600'}`} style={{ animationDelay: '0.5s' }} />
            </motion.div>
            <p className={`text-center text-xs mb-4 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Instant login • No credentials required</p>
            ...
            <div className="grid grid-cols-3 gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="group relative px-3 py-4 rounded-2xl bg-gradient-to-br from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 border-2 border-indigo-400/50 text-sm font-bold text-white shadow-lg shadow-indigo-500/40 transition-all overflow-hidden"
              >
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.08, y: -2 }}
                whileTap={{ scale: 0.95 }}
                className="group relative px-3 py-4 rounded-2xl bg-gradient-to-br from-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-600 border-2 border-purple-400/50 text-sm font-bold text-white shadow-lg shadow-purple-500/40 transition-all overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
                <div className="relative flex flex-col items-center gap-2">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.08, y: -2 }}
                whileTap={{ scale: 0.95 }}
                className="group relative px-3 py-4 rounded-2xl bg-gradient-to-br from-cyan-600 to-cyan-700 hover:from-cyan-500 hover:to-cyan-600 border-2 border-cyan-400/50 text-sm font-bold text-white shadow-lg shadow-cyan-500/40 transition-all overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
                <div className="relative flex flex-col items-center gap-2">
        ...
        <motion.div variants={itemVariants} className={`text-center text-sm pt-2 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
          ...
          <Link to="/register" className={`font-semibold underline decoration-indigo-400/30 hover:decoration-indigo-400 underline-offset-4 transition-all ${isDark ? 'text-indigo-400 hover:text-indigo-300' : 'text-indigo-600 hover:text-indigo-700'}`}>
            Register here
          </Link>
        </motion.div>
      </motion.div>
